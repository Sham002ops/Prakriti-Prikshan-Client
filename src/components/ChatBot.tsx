...
export default function ChatBot({ isOpen, onClose }: ChatBotProps) {
  ...
  useEffect(() => {
    ...
    if (!token) {
      append({ sender: "system", text: "No authentication token found. Please sign in." });
      return;
    }

    // FIX: Get backend URL from environment variable
    const backendUrl = import.meta.env.VITE_BACKEND1_URL || 'https://api.shamband.work';
